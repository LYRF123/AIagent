...
ACRONYM_QUERY_PATTERN = re.compile(r"[A-Za-z][A-Za-z0-9-]{1,12}")
SEPARATED_ACRONYM_QUERY_PATTERN = re.compile(
    r"(?<![A-Za-z0-9])(?:[A-Za-z]\s*(?:[.\-]\s*|\s+)){2,}[A-Za-z](?![A-Za-z0-9])"
)
ACRONYM_DEFINITION_PATTERN = re.compile(
    r"([A-Z][A-Za-z]+(?:[-\s]+[A-Z][A-Za-z]+){1,8})\s*\(([A-Z][A-Z0-9-]{1,12})\)"
)
QUERY_SYNONYMS = {
...
class QueryExpander:
    def __init__(self, corpus: PaperCorpus) -> None:
        ...
        self.acronym_definitions = self._extract_acronym_definitions(corpus)

    def _extract_acronym_definitions(self, corpus: PaperCorpus) -> dict[str, str]:
        definitions: dict[str, str] = {}
        for paper in corpus.papers:
            text = f"{paper.title}\n{paper.summary[:4000]}"
            for phrase, acronym in ACRONYM_DEFINITION_PATTERN.findall(text):
                key = acronym.lower()
                compact_phrase = " ".join(phrase.replace("-", " ").split())
                if key not in definitions and compact_phrase:
                    definitions[key] = compact_phrase
        return definitions

    @staticmethod
    def _extract_query_acronyms(query: str) -> list[str]:
        acronyms: list[str] = []
        for token in ACRONYM_QUERY_PATTERN.findall(query):
            acronyms.append(token.lower())
        for token in SEPARATED_ACRONYM_QUERY_PATTERN.findall(query):
            compact = re.sub(r"[^A-Za-z0-9]", "", token).lower()
            if compact:
                acronyms.append(compact)
        return acronyms
    ...
    def expand(self, query: str, limit: int = 4) -> list[str]:
        ...
        acronym_tokens = self._extract_query_acronyms(normalized)

        for token in list(dict.fromkeys(tokens + acronym_tokens)):
            synonym = QUERY_SYNONYMS.get(token)
            if synonym:
                variants.append(f"{normalized} {synonym}".strip())
            acronym_definition = self.acronym_definitions.get(token)
            if acronym_definition and acronym_definition.lower() not in normalized.lower():
                variants.append(f"{normalized} {acronym_definition}".strip())
